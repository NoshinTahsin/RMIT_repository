...
def convertDocToPdf(DOC_FILEPATH):
...
#
	with open("someng.txt", "a+") as f:
		f.write("abstract")
		f.write('\n"\n')
	f.close()
	with open("someng.txt", "ab+") as f:
...
if __name__ == '__main__':
    ...
    texts = list(pd.read_csv('outputimagetess.txt')['abstract'])
    
    #texts = list(pd.read_csv('data/example.txt', error_bad_lines= False,quoting=csv.QUOTE_NONE)['abstract'])
    
    print("anikaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\n")
    print(texts[0])
    print(len(texts))
